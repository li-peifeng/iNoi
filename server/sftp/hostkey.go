...
func InitHostKey() {
	...
	if !utils.Exists(sshPath) {
		...
		if err != nil {
			utils.Log.Errorf("创建 SSH 目录失败: %+v", err)
			return
		}
	}
...
func LoadOrGenerateRSAHostKey(parentDir string) (ssh.Signer, bool) {
	...
	if err != nil {
		utils.Log.Errorf("生成 RSA 私钥失败: %+v", err)
		return nil, false
	}
	publicKey, err := ssh.NewPublicKey(&privateKey.PublicKey)
	if err != nil {
		utils.Log.Errorf("生成 RSA 公钥失败: %+v", err)
		return nil, false
	}
	ret, err := ssh.NewSignerFromKey(privateKey)
	if err != nil {
		utils.Log.Errorf("生成 RSA 签名器失败: %+v", err)
		return nil, false
	}
	...
	if err != nil {
		utils.Log.Errorf("写入 RSA 私钥文件失败: %+v", err)
		return nil, false
	}
	err = os.WriteFile(publicKeyPath, publicBytes, 0644)
	if err != nil {
		_ = os.Remove(privateKeyPath)
		utils.Log.Errorf("写入 RSA 公钥文件失败: %+v", err)
		return nil, false
	}
	return ret, true
}
