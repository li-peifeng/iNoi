...
func Start() {
	...
	if conf.Conf.Scheme.HttpPort != -1 {
		...
		fmt.Printf("启动 HTTP 服务器 @ %s\n", httpBase)
		utils.Log.Infof("启动 HTTP 服务器 @ %s", httpBase)
		...
		go func() {
			...
			if err != nil && !errors.Is(err, http.ErrServerClosed) {
				...
				utils.Log.Errorf("启动 HTTP 服务器失败: %s", err.Error())
			} else {
	...
	if conf.Conf.Scheme.HttpsPort != -1 {
		...
		fmt.Printf("启动 HTTPS 服务器 @ %s\n", httpsBase)
		utils.Log.Infof("启动 HTTPS 服务器 @ %s", httpsBase)
		...
		go func() {
			...
			if err != nil && !errors.Is(err, http.ErrServerClosed) {
				...
				utils.Log.Errorf("启动 HTTPS 服务器失败: %s", err.Error())
			} else {
				handleEndpointShutdownHooks("https")
			}
		}()
		if conf.Conf.Scheme.EnableH3 {
			fmt.Printf("启动 HTTP3 (quic) 服务器 @ %s\n", httpsBase)
			utils.Log.Infof("启动 HTTP3 (quic) 服务器 @ %s", httpsBase)
			...
			go func() {
				...
				if err != nil && !errors.Is(err, http.ErrServerClosed) {
					...
					utils.Log.Errorf("启动 HTTP3 (quic) 服务器失败: %s", err.Error())
				} else {
	...
	if conf.Conf.Scheme.UnixFile != "" {
		fmt.Printf("启动 Unix 服务器 @ %s\n", conf.Conf.Scheme.UnixFile)
		utils.Log.Infof("启动 Unix 服务器 @ %s", conf.Conf.Scheme.UnixFile)
		unixSrv = &http.Server{Handler: httpHandler}
		go func() {
			listener, err := net.Listen("unix", conf.Conf.Scheme.UnixFile)
			if err != nil {
				utils.Log.Errorf("启动 Unix 服务器失败: %+v", err)
				return
			}
			unixRunning = true
			...
			if err != nil {
				utils.Log.Errorf("解析 Unix 套接字文件权限失败: %+v", err)
			} else {
				err = os.Chmod(conf.Conf.Scheme.UnixFile, os.FileMode(mode))
				if err != nil {
					utils.Log.Errorf("修改 Unix 套接字文件权限失败: %+v", err)
				}
			}
			...
			if err != nil && !errors.Is(err, http.ErrServerClosed) {
				...
				utils.Log.Errorf("启动 Unix 服务器失败: %s", err.Error())
			} else {
	...
	if conf.Conf.S3.Port != -1 && conf.Conf.S3.Enable {
		...
		fmt.Printf("启动 S3 服务器 @ %s\n", s3Base)
		utils.Log.Infof("启动 S3 服务器 @ %s", s3Base)
		go func() {
			s3Running = true
			var err error
			if conf.Conf.S3.SSL {
			...
			if err != nil && !errors.Is(err, http.ErrServerClosed) {
				...
				utils.Log.Errorf("启动 S3 服务器失败: %s", err.Error())
			} else {
	...
	if conf.Conf.FTP.Listen != "" && conf.Conf.FTP.Enable {
		...
		if err != nil {
			utils.Log.Errorf("启动 FTP 驱动失败: %s", err.Error())
		} else {
			fmt.Printf("启动 FTP 服务器 @ %s\n", conf.Conf.FTP.Listen)
			utils.Log.Infof("启动 FTP 服务器 @ %s", conf.Conf.FTP.Listen)
			go func() {
				...
				if err != nil {
					...
					utils.Log.Errorf("启动 FTP 服务器失败: %s", err.Error())
				} else {
	...
	if conf.Conf.SFTP.Listen != "" && conf.Conf.SFTP.Enable {
		...
		if err != nil {
			utils.Log.Errorf("启动 SFTP 驱动失败: %s", err.Error())
		} else {
			fmt.Printf("启动 SFTP 服务器 @ %s\n", conf.Conf.SFTP.Listen)
			utils.Log.Infof("启动 SFTP 服务器 @ %s", conf.Conf.SFTP.Listen)
			go func() {
				...
				if err != nil {
					...
					utils.Log.Errorf("启动 SFTP 服务器失败: %s", err.Error())
				} else {
	...
	if conf.Conf.FTP.Listen != "" && conf.Conf.FTP.Enable && ftpServer != nil {
		...
		go func() {
			...
			if err := ftpServer.Stop(); err != nil {
				utils.Log.Error("FTP 服务器关闭错误: ", err)
			}
			ftpServer = nil
		}()
	}
	if conf.Conf.SFTP.Listen != "" && conf.Conf.SFTP.Enable && sftpServer != nil {
		...
		go func() {
			...
			if err := sftpServer.Close(); err != nil {
				utils.Log.Error("SFTP 服务器关闭错误: ", err)
			}
			sftpServer = nil
			sftpDriver = nil
		}()
